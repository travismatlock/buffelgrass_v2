...
ui <- fluidPage(
  ...
  titlePanel(paste0("Rainfall-Event-Based Buffelgrass Forecast for ",as.character(today)," -- Beta Version")), # App title
  
  
  sidebarLayout(
    sidebarPanel(
      textOutput("description")), # Description of leaflet on left of page
    mainPanel(leafletOutput("mymap", width="100%", height="155%"), imageOutput("logo"))))
...
server <- function(input, output) {
  ...
  output$description <- renderText('This map displays the number of precipitation events in a 30-day rolling window
                                    prior to today. It is a product of the USA National Phenology Network
                                    ...
                                    (more at usanpn.org/data/forecasts/Buffelgrass). Clicking on points will
                                    display the source and value of that gauge. A value of 4 events is interpreted
                                    as likely green-up within two weeks. The map combines PRISM raster and NOAA 
                                    RCC-ACIS and RainLog spatial point precipitation data. Rainfall must exceed 0.25"
                                    in a single calendar day to be counted as an "event". Each following day with rainfall
                                    above 0.25" is counted in the same single event. Following this, a 
                                    minimum of 3 consecutive days with precipitation under 0.25" 
                                    is required for the next event to be counted. 
